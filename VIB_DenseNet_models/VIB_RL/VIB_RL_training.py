...
save_root = "/data/zhouabx/KDEF/KDEF_RDM_new/reward_only/2023_May_Reward/CelebA_train_results"
#reward only
save_freq = 1
model.train()
loss_memory_reward = []
...
if epoch % save_freq == save_freq - 1:
    savename = f'only_reward.ckp'
    save_path = os.path.join(save_root, savename)
    torch.save({
        'epoch_reward': epoch,
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
        'loss_memory_reward': loss_memory_reward,
        }, save_path)
...
def RDM(df):
    ...
    # reward RDM
    nimages = len(flat_allrewards)
    modelRDM_VIB_representation = np.zeros((nimages,nimages))
    for i in range(nimages):
        for j in range(nimages):
            modelRDM_VIB_representation[i,j] = 1 - stats.pearsonr(flat_allmus[i], flat_allmus[j])[0]
       
    #true reward values & kendalltau values
    v_RDM_true_reward = RDM_true_reward[np.triu_indices(RDM_true_reward.shape[0], 1)]
    v_modelRDM_VIB_representation = modelRDM_VIB_representation[np.triu_indices(modelRDM_VIB_representation.shape[0], 1)]
    kendalltau_TrueReward_reward = stats.kendalltau(v_RDM_true_reward, v_modelRDM_VIB_representation)
   
    res = {"RDM_true_reward": RDM_true_reward,
           "VIB_representation": modelRDM_VIB_representation,
           "KT_TrueReward_reward": kendalltau_TrueReward_reward}    
    return res  
...
RDM_true_reward = []
VIB_representation = []
KT_TrueReward_reward = []
for _ in range(10):   
    RDM_true_reward.append(RDM(df)['RDM_true_reward'])
    VIB_representation.append(RDM(df)['VIB_representation'])
    KT_TrueReward_reward.append(RDM(df)['KT_TrueReward_reward'])

#save true reward values & kendalltau results 
path = "/data/zhouabx/KDEF/KDEF_RDM_new/reward_only/2023_May_Reward/True_reward_results/"
with open(path + 'RDM_true_reward_values.pkl', 'wb') as pickle_out:
    pickle.dump(RDM_true_reward, pickle_out)   

path = "/data/zhouabx/KDEF/KDEF_RDM_new/reward_only/2023_May_Reward/True_reward_results/"
with open(path + 'VIB_representation_values.pkl', 'wb') as pickle_out:
    pickle.dump(VIB_representation, pickle_out) 

path = "/data/zhouabx/KDEF/KDEF_RDM_new/reward_only/2023_May_Reward/True_reward_results/"
with open(path + 'KT_TrueReward_reward.pkl', 'wb') as pickle_out:
    pickle.dump(KT_TrueReward_reward, pickle_out)     
